...
// Virtual button codes - they are used to represent buttons on keyboard or controls in simulator window.
// They are translated into design-specific values from guiCore.h before calling guiTop event handlers
enum buttons {
    BTN_ESC = 1,
    BTN_LEFT,
    BTN_RIGHT,
    ...
    ui(new Ui::MainWindow)
{
    ...
    // Here GUI may be drawn for the first time - depends on your preferences
    // A convinient way for simulator is to draw GUI processing some event - buttons,
    // touch or encoder (or whatever it is)
    on_LCD_update();
}
...
void MainWindow::updateDisplayWrapper(void *buffer)
{
